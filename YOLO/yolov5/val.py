...
def run(data,
    ...
    if test==False:
        ...
        jdict, stats, ap, ap_class = [], [], [], []
        pbar = tqdm(dataloader, desc=s, ncols=NCOLS, bar_format='{l_bar}{bar:10}{r_bar}{bar:-10b}')  # progress bar
        # for i, (image,file_names) in enumerate(dataloader):
        #     print(image.shape)
        #     print(file_names)
        ...
        for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
            ...
            #     img.save('/mnt/yj/detection/temp/{:05d}.jpg'.format(batch_i*128+j))
            t1 = time_sync()
            if pt:
                im = im.to(device, non_blocking=True)
                targets = targets.to(device)
        ...
        #             pred[:, 5] = 0
                ...
                mask = predn[:,4] > 0.3
                # print(mask)
                boxes = predn[:,:4][mask].detach().cpu().numpy()
                scores = predn[:,4][mask].detach().cpu().numpy()
                class_ids = predn[:,5][mask].detach().cpu().numpy()
                # print(class_ids)
                for box, score, class_id in zip(boxes, scores, class_ids):
                    x_min, y_min, x_max, y_max = box
                    file_name = 'test_{:06d}.json'.format(128*batch_i+si+200000)
                    results['file_name'].append(file_name)
                    results['class_id'].append(int(class_id))
        ...
        submission.to_csv('/mnt/yj/detection/yolov5/submission.csv', index=False)
        submission.head(10)
